...
public:
  ...
  * implementation.
  ...
  void compute_generalized_eigenvalues_symmetric (
    ...
    const int itype = 1);
  /*
    Exactly same to above one, expect for returning the error
    information when calling solving EVP if there is any
   */
  int compute_generalized_eigenvalues_symmetric_returnInfo (
    LAPACKFullMatrix<number> &B,
    std::vector<Vector<number> > &eigenvectors,
    const int itype = 1);
