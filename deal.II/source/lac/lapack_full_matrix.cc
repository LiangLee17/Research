// ---------------------------------------------------------------------
// $Id: lapack_full_matrix.cc 30037 2013-07-18 16:55:40Z maier $
//
  ...
  const int itype)
{
  ...
  Assert (info >=0, ExcInternalError());
  if (info != 0)
    std::cerr << "LAPACK error in sygvx: info = " << info << std::endl;
  ...
  const int itype)
{
  ...
  Assert (info >=0, ExcInternalError());
  if (info != 0)
    std::cerr << "LAPACK error in sygv = " << info << std::endl;
...
template <typename number>
int
LAPACKFullMatrix<number>::compute_generalized_eigenvalues_symmetric_returnInfo (
  LAPACKFullMatrix<number> &B,
  std::vector<Vector<number> > &eigenvectors,
  const int itype)
{
